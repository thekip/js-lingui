...
const renderNodes = (
  ...
  const emptyChildrenButNeedsHandling =
    targetString &&
    new RegExp(keepArray.map((keep) => `<${keep}`).join("|")).test(targetString)
  ...
  const getData = (childs) => {
    ...
    childrenArray.forEach((child) => {
      ...
      else if (isObject(child) && !isValidElement(child))
...
export function Trans({
  ...
  const reactI18nextOptions = { ...i18n.options?.react }
